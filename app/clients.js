...
const checkGlueConnection = () => {
    ...
    const toggleGlueAvailable = (available) => toggleStatusLabel('glueSpan', 'Glue is', available);

    glue.connection.connected(() => toggleGlueAvailable(true));

    glue.connection.disconnected(() => toggleGlueAvailable(false));
};
...
const setUpUi = () => {
    ...
    if (portfolioButton) {
        ...
        portfolioButton.onclick = () => openWindow('Portfolio', glue.windows.my(), 'bottom');
    }
    ...
    const setUpFrameButtonClick = () => {
        ...
        currentWindow.onFrameButtonClicked((buttonInfo) => {
...
const getWindowDirection = () => {
    ...
    const workingAreaHeight = primaryMonitor.workingAreaHeight;
    const top = currentWindow.bounds.top;
    const height = currentWindow.bounds.height;

    const workingHeightDiff = workingAreaHeight - (top + height);

    if (workingHeightDiff >= 400) {
        return 'bottom';
    }
