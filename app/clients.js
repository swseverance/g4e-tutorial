...
const setUpUi = () => {
    ...
    const setUpFrameButtonClick = () => {
        ...
        myWindow.onFrameButtonClicked((btnInfo) => {
            if (btnInfo.buttonId === 'portfolio-btn') {
...
const registerGlueMethods = () => {
    ...
    glue.agm.register({
            ...
            description: 'Finds the appropriate clients that have to be called.',
            accepts: 'String symbol',
        },
        (symbol) => glue.windows.open(
            'Find Who To Call',
...
const openTabWindow = (party, direction) => {
    ...
    const groupExists = glue.windows.list().some((window) => window.tabGroupId === 'PortfolioTabs');
    if (!groupExists) {
