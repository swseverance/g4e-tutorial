...
const setUpUi = () => {
    ...
    const setUpPortfolioFrameButton = () => {
        ...
        // SOLVED TUTOR_TODO Chapter 4.4 Task 1
        const buttonOptions = {
...
const trackTheme = () => {
    ...
    // SOLVED TUTOR_TODO Chapter 10 task 1
...
const getWindowDirection = () => {
    ...
    // SOLVED TUTOR_TODO Chapter 4.4 Task 2
    const primaryWin = glue42gd.monitors.filter(m => m.isPrimary === true)[0];
    const btmNeighbours = glue.windows.my().bottomNeighbours;
    const workingArea = primaryWin.workingAreaHeight;
    const windowBottom = glue.windows.my().bounds.top + glue.windows.my().bounds.height;
    if (windowBottom + 400 < workingArea && btmNeighbours.length === 0) {
        return 'bottom';
    }
    return 'right';
