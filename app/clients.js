...
const setUpUi = () => {
    ...
    if (portfolioButton) {
        ...
        portfolioButton.onclick = () => {
            ...
            openWindow('Portfolio', myWin, direction);
    ...
    const setUpPortfolioFrameButton = () => {
        ...
        glue.windows.my().addFrameButton(buttonOptions);
    ...
    console.log(glue.activities.inActivity);
...
const setupClients = () => {
    ...
    const addRow = (table, client) => {
        ...
        row.onclick = () => {
            ...
            // TUTOR_TODO Chapter 11 - check if you are in an activity and either update the activity context or
            // open a tab window and invoke the agm method
...
const getWindowDirection = () => {
    ...
    // console.log(htmlContainer.monitors);
    ...
    const topFreeSpace = myWindow.bounds.top;
    const windowHeight = myWindow.bounds.height;
    const bottomNeighboursHeight = bottomNeighbours.reduce((sum, window) => sum + window.bounds.height, 0);

    const bottomFreeSpace = workingAreaHeight - (topFreeSpace + windowHeight + bottomNeighboursHeight);
    console.log(bottomFreeSpace);
    return bottomFreeSpace < 400 ? 'right' : 'bottom';
...
const openWindow = (windowName, myWin, direction) => {
    ...
    // TUTOR_TODO Chapter 4.1 - create an options object and define modens object and define mode, relativeTo and relativeDirection properties
    ...
    //     context: {
    //         parentWindowId: glue.windows.my().id,
    //     },
    // }
...
const openTabWindow = (party, direction) => {
    ...
    const portfolioTabsExists = glue.windows.list().some(window => window.tabGroupId === 'PortfolioTabs');
    if (!portfolioTabsExists) {
        windowSettings.relativeTo = glue.windows.my().id; // glue.windows.my()
        windowSettings.relativeDirection = direction;
    }

    // glue.windows.find('PortfolioTabs', (window) => {}, 
    //     (err) => {
    //         options.relativeTo = glue.windows.my().id;
    //         options.relativeDirection = "right";
    //     }
    // )

    // glue.windows.open(party.name, window.location.href.replace('clients.html', 'portfolio.html'), options);
