...
// REVIEW: Errors here are not handled
...
const initInstrumentSearch = () => {
    ...
    const clientSearch = new gssClientSearch.create(gssOptions);
...
const registerAgmMethod = () => {
    ...
    // REVIEW: extract in a variable to improve readability
...
const setUpWindowEventsListeners = () => {
    ...
    // REVIEW: how do we name variables?
...
const setUpTabControls = () => {
    ...
    if (window.tabs.length > 1) {
        ...
        window.addFrameButton(extractTab)
    ...
    // REVIEW: improve expressions
    glue.windows.my().onWindowAttached((win) => {
        win.addFrameButton(extractTab)
            .catch((error) => console.log(error));

        win.removeFrameButton(gatherTab.buttonId)
            .catch((error) => console.log(error));
    });
    ...
    glue.windows.my().onDetached((win) => {
        win.addFrameButton(gatherTab)
            .catch((error) => console.log(error));

        win.removeFrameButton(extractTab.buttonId)
            .catch((error) => console.log(error));
    });

    glue.windows.my().onWindowDetached((win) => {
        const firstTabWindow = window.tabs[0];
        if (firstTabWindow) {
            window.addFrameButton(extractTab)
                .catch((error) => console.log(error));
        }
    ...
    glue.windows.my().onFrameButtonClicked((buttonInfo) => {
        ...
        const clientsID = glue.windows.my().context.id;
        const clientsWindow = glue.windows.findById(clientsID);

        if (buttonType === 'extractTabs') {
...
const sendPortfolioAsEmailClicked = (event) => {
    ...
    const sendPortfolioAsEmail = (client, portfolio) => {
        ...
        // REVIEW: here expression is ok - consistency?
