...
const setUpUi = () => {
    ...
    const setUpPortFolioFrameButtons = () => {
        glue.windows.my().addFrameButton({
    ...
    const setUpFrameButtonClick = () => {
        glue.windows.my().onFrameButtonClicked((buttonInfo, win) => {

            if (buttonInfo.buttonId !== 'portfolio-btn') {
                return;
            }

            const primaryMonitor = htmlContainer.monitors.filter((monitor) => monitor.isPrimary)[0];

            if (win.bottomNeighbours.length === 0 && primaryMonitor.workingAreaHeight >= win.bounds.top + win.bounds.height + 400) {
                openWindow('Portfolio', glue.windows.my(), 'bottom');
            } else {
                openWindow('Portfolio', glue.windows.my(), 'right');
            }
        })
    };
...
const setupClients = () => {
    ...
    const addRow = (table, client) => {
        ...
        row.onclick = () => {
            ...
            if (inActivity) {
                glue.activities.my.activity.updateContext({
                    party: client
                });
            } else {
...
const registerGlueMethods = () => {
    glue.agm.register('GWM.FindWhoToCall',
        (args) => {
            const symbol = args.symbol;

            const popup = window.location.href.replace('clients.html', 'symbolPopup.html');

            glue.windows.open('symbol-popup-' + symbol, popup, {
                focus: false,
                context: {
                    symbol: symbol
                }
...
const invokeAgMethod = (client) => {
    ...
    const target = 'all';
    glue.agm.invoke(MethodName, invocationArgs, target)
        .then((result) => {
            console.log('invoked');
        })
        .catch((err) => {
...
const openTabWindow = (party) => {
    ...
    // const isTabContainerOpened = glue.windows.list().filter((w) => w.name.indexOf('PortfolioTabs_') !== -1);
    ...
    // if (isTabContainerOpened.length === 0) {
    ...
    const isTabContainerOpened = glue.windows.list().filter((w) => w.tabGroupId === windowSettings.tabGroupId);

    if (isTabContainerOpened.length === 0) {
        windowSettings.relativeTo = glue.windows.my().id;
        windowSettings.relativeDirection = 'right';
    }
